...
| ランナー | サンドボックス / `runner` ユーザー | リポジトリ全体の所有、git、sudo なし | tasks.json 全体、台帳、テスト結果 |
| ソルバー | サンドボックス / `solver` ユーザー | 限定的な書き込みのみ | ランナーが組み立てた1ステップ分のブリーフだけ |
...
1. **ソルバーの実体** — ディストロには Codex CLI と Claude Code が両方入っているが、
   **どちらも未認証**（2026-08-17 時点）。`--uid-owner solver` での egress 制限下で動くかも未検証。
   加えて認証方式が問題になる: どちらの CLI も既定は対話ログインで、認証情報を
   **実行ユーザーの home に書く**。人間がログインしたのは `maint` なので、`solver` は
   それを読めない（読めたら §1-2 が壊れる）。`solver` には**非対話で使える専用の API キー**を
   環境変数で渡す必要がある（サンドボックス専用・失効可能・上限額つき）
