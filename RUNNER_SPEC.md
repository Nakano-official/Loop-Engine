...
| 主体 | 実体 | 実行場所 | 権限 | 見えるもの |
|---|---|---|---|---|
| プランナー | **Claude Code** | **ホスト (Windows)** | — | 人間との対話、`plan/` の生成、ESCALATION.md の判定 |
| ランナー | **このプログラム**（AI ではない） | サンドボックス / `runner` ユーザー | リポジトリ全体の所有、git、sudo なし | tasks.json 全体、台帳、テスト結果 |
| ソルバー | **Codex CLI** | サンドボックス / `solver` ユーザー | 限定的な書き込みのみ | ランナーが組み立てた1ステップ分のブリーフだけ |

三者のうち **AI は両端の2つだけ**。真ん中のランナーは決まった手順を実行するだけなので、
プログラムで書く。判断を AI に委ねると「まあ通ったことにする」が起こり得るが、
関門の値打ちは**判断しないこと**にあるため。

サンドボックスは WSL2 のディストロ（§1-1）。保守用の `maint`（sudo あり）も同じ
ディストロにいるが、ループの三者には含まれない。

**プランナーはサンドボックスに入らない。ソルバーは plan/ を書けない。**
この2つが崩れると以下の全部が意味を失う。

前者は**規律であって機構ではない**。Windows からは `wsl -u root` でいつでも中の root に
なれるので、境界は片方向にしか立っていない ── 守っているのは「サンドボックスから
Windows 側へ手が伸びない」方向だけである（§1-4）。
...
STUB のブリーフには**シグネチャ以外を一切載せない**。意味を運ぶものを載せると
...
最上位（ステップの外）に置ける設定が2つある。どちらも省略可で、
...
1. **ソルバーの動作条件** — ソルバーの実体は **Codex CLI に確定**（2026-08-17）。
   プランナーは Claude Code（ホスト側）。残っているのは実体ではなく、次の3点:
   - **認証の渡し方**: Codex CLI の既定は対話ログインで、認証情報を**実行ユーザーの
     home に書く**。人間がログインするのは `maint` だが実際に走るのは `solver` で、
     `solver` は `maint` の home を読めない（読めたら §1-2 が壊れる）。したがって
     **非対話で使える専用の API キーを環境変数で渡す**（サンドボックス専用・失効可能・
     上限額つき）。2026-08-17 時点で未発行
   - **egress 制限下で動くか**: `--uid-owner solver` で通信先を絞った状態は未検証
   - **エンドポイントの特定**: `60-egress.sh` は解決した IP を固定するので、**推測で
     ドメインを列挙しない**。制限をかける前に一度動かして実際の通信先を確認する
